...
class FusionHTTPFileSystem(HTTPFileSystem):  # type: ignore
    ...
    async def _download_single_file_async_with_checksum(
        ...
        try:
            ...
            await loop.run_in_executor(None, create_parent_dir)

            def write_file_data() -> None:
                with Path(output_path).open("wb") as f:
                    f.write(file_data)

            await loop.run_in_executor(None, write_file_data)
    ...
    def stream_single_file_with_checksum_validation(  # noqa: PLR0915
        ...
        def stream_and_validate() -> Tuple[bool, str, Optional[str]]:
    ...
    def _crc32(data: bytes) -> int:
        """Compute CRC32 (Ethernet, gzip) checksum.
        
        Uses the CRC-32 polynomial (0xEDB88320 - bit-reflected form of 0x04C11DB7).
        This is an implementation for Python 3.7 compatibility.
        ...
        """
        # CRC-32 polynomial (Ethernet, gzip) - bit-reflected
        poly = 0xEDB88320
        crc = 0xFFFFFFFF
    ...
    def _crc32c(data: bytes) -> int:
        """Compute CRC32C (Castagnoli, iSCSI) checksum.
        
        Uses the CRC-32C polynomial (0x82F63B78 - bit-reflected form of 0x1EDC6F41).
        This is an implementation for Python 3.7 compatibility.
        ...
        Returns:
            32-bit CRC value as integer
        """
        # CRC-32C polynomial (Castagnoli) - bit-reflected
        poly = 0x82F63B78
        crc = 0xFFFFFFFF
    ...
    def _crc64nvme(data: bytes) -> int:
        """Compute CRC-64/NVME checksum.
        
        Uses the CRC-64/NVME polynomial (0x9A6C9329AC4BC9B5) which is used by NVMe specification.
        This is an implementation since awscrt doesn't support it in Python 3.7 compatible versions.
        
        Args:
            data: Input bytes to compute CRC for
            
        Returns:
            64-bit CRC value as integer
        """
    ...
    def _compute_checksum_from_data(self, data: bytes, algorithm: str, is_multipart: bool = False) -> str:  # noqa: PLR0911, PLR0912
        ...
        """
        if algorithm == "CRC32":
            crc_value = self._crc32(data)
            crc_bytes = crc_value.to_bytes(4, byteorder="big")

            if is_multipart:
                composite_crc = self._crc32(crc_bytes)
            ...
            crc_value = self._crc32c(data)
            crc_bytes = crc_value.to_bytes(4, byteorder="big")

            if is_multipart:
                composite_crc = self._crc32c(crc_bytes)
                return base64.b64encode(composite_crc.to_bytes(4, byteorder="big")).decode("ascii")
            else:
                return base64.b64encode(crc_bytes).decode("ascii")
            ...
            crc_value = self._crc64nvme(data)
            crc_bytes = crc_value.to_bytes(8, byteorder="big")

            if is_multipart:
                composite_crc = self._crc64nvme(crc_bytes)
    ...
    def get(  # disable: W0221
        ...
        lpath: Union[str, io.IOBase, fsspec.spec.AbstractBufferedFile, Path],
        chunk_size: int = 5 * 2 ** 20,
        **kwargs: Any,
    ) -> Any:
        ...
        if n_threads == 1 or not is_local_fs:
            ...
            try:
                ...
                async def get_content_length_and_checksum() -> Tuple[Optional[int], Optional[str], Optional[str]]:
                    session = await self.set_session()
                    async with session.head(rpath, **self.kwargs) as r:
                        r.raise_for_status()
