...
class FusionEmbeddingsConnection(Connection):  # type: ignore
    ...
    def __init__(  # noqa: PLR0912, PLR0913, PLR0915
        ...
        **kwargs: Any,
        ...
        credentials: Union[FusionCredentials, str, None] = kwargs.get("credentials", "config/client_credentials.json")
    ...
    @staticmethod
    def _modify_post_response_langchain(
        raw_data: Union[str, bytes, bytearray]
    ) -> Union[str, bytes, bytearray]:
        """Modify the response from langchain POST request to match the expected format.

        Args:
            raw_data (Union[str, bytes, bytearray]): Raw post response data.

        Returns:
            Union[str, bytes, bytearray]: Modified post response data.
        """
        if len(raw_data) > 0:
            try:
                ...
                if "hits" in data:
                    ...
                    # Wrap the existing "hits" list in another dictionary with the key "hits"
    ...
    @staticmethod
    def _modify_post_haystack(
        body: Optional[bytes], method: str
    ) -> Optional[bytes]:
        ...
        Returns:
            Optional[bytes]: Modified request body.
        """
    ...
    def perform_request(  # noqa: PLR0913
        ...
        method: str,
        url: str,
        params: Optional[Mapping[str, Any]] = None,  # noqa: ARG002
...
def format_index_body(
    number_of_shards: int = 1, number_of_replicas: int = 1, dimension: int = 1536
) -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: Index body expected by embeddings API.
    """
    index_body = {
        "settings": {
            "index": {
